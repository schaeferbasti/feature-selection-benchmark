...
def main():
    ...
    for f in result_files:
        if f.__contains__("filter"):
            dataset_id = f.split('.')[0].split('/')[-1].split('_')[-1]
        elif f.__contains__("metalearning"):
            dataset_id = f.split('.')[0].split('/')[-1].split('_')[-1]
        else:
            dataset_id = f.split('.')[0].split('/')[-1]
    ...
    for dataset_id, files in dataset_files.items():
        ...
        # === ORIGINAL RESULTS ===
        original_path = f"data/original/Original_{dataset_id}.parquet"
        try:
        ...
        for data_file in files:
            if "OpenFE" in data_file:
                ...
                method = name.split('/')[-1]
                result_path = f"results/{method}.parquet"
                try:
