...
def main(dataset_id, seed):
    ...
    try:
        data = pd.read_parquet("data/wrapper/SkrebateSURF_" + str(dataset_id) + ".parquet")  # ../
        print("File exists" + str(data.head()) + "\n\n")
    except FileNotFoundError:
        print("Calculate Feature Selection")
        clf = make_pipeline(SURF(n_features_to_select=int(len(X_train.columns)/2)), RandomForestClassifier(n_estimators=100))
        ...
        data.to_parquet("data/wrapper/SkrebateSURF_" + str(dataset_id) + ".parquet")  # ../
